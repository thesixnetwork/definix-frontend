        ...
        trade.route.path.map((token, i, path) => {
          ...
          return (
            <Flex
              ...
              mb={isMobile ? '8px' : '0px'}
            >
              <Coin size={isMobile ? '20px' : '22px'} symbol={token?.symbol} mr={isMobile ? '9px' : '6px'} />
              <Text textStyle="R_14M" color={ColorStyles.DEEPGREY} mr={isLastItem ? '0px' : isMobile ? '14px' : '10px'}>
                {token.symbol}
              </Text>
              {!isLastItem && (
                <Flex>
