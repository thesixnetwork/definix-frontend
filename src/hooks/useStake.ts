...
const useStake = (pid: number) => {
  ...
  const handleStake = useCallback(
    async (amount: string) => {
      ...
      if (connector === "klip") {
        setShowModal(true)
        console.log("klip log amount : ", new BigNumber(amount).times(new BigNumber(10).pow(18)).toString(), " pid : ", pid)
        if (pid === 0) {
  ...
  const handleStake = useCallback(
    async (amount: string) => {
      if (connector === "klip") {
        setShowModal(true)
        if (sousId === 0) {
          klipProvider.genQRcodeContactInteract(herodotusContract._address,jsonConvert(getAbiHerodotusByName("enterStaking")),jsonConvert([new BigNumber(amount).times(new BigNumber(10).pow(18)).toString()]))
        } else {
          klipProvider.genQRcodeContactInteract(herodotusContract._address,jsonConvert(getAbiHerodotusByName("deposit")),jsonConvert([sousId,new BigNumber(amount).times(new BigNumber(10).pow(18)).toString()]))
        }
