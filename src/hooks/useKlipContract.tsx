...
class KlipConnector {
  ...
  private async _getRequestKey({ contractAddress, abi, input, value }: KlipContractProps) {
    try {
      ...
      return axios.post(`${KLIP_BASE_URL}/prepare`, data)
        .then(({ data }) => {
  ...
  private _onRenderingInterval(expireDuration: Dayjs) {
    ...
    // @ts-ignore
    document.querySelector('.' + this._options.intervalClassName)?.innerHTML = this._options.renderTimeFormat(expireDuration.valueOf())
  }
