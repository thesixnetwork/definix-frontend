...
const SliderSection = styled(Box)<{ curTheme: any }>`
  ...
  .slick-slider {
    padding-bottom: ${spacing.S_20 + 5}px;
  }
  ...
  .slick-dots {
    position: absolute;
    bottom: ${spacing.S_20}px;
    height: auto;
    > li {
      padding-right: ${spacing.S_6}px;
      width: 5px;
      height: 5px;
      > button {
        width: inherit;
...
const SlideItem = styled(Box)<{ index: number; curTheme: any }>`
  margin: ${spacing.S_20}px 0;
  padding-left: ${({ index }) => (index > 0 ? spacing.S_32 : spacing.S_40)}px;
  width: ${({ index }) => (index > 0 ? '277' : '276')}px;
  border-left: ${({ index, curTheme, theme }) => (index > 0 ? `1px solid ${curTheme.borderColor}` : 'none')};

  ${mediaQueries.mobileXl} {
    margin: 0;
    padding-left: 0;
    padding-right: 0;
    /* padding-top: ${({ theme, index }) => spacing[index > 0 ? 'S_20' : 'S_16']}px; */
    padding-top: ${spacing.S_16}px;
    padding-bottom: ${spacing.S_16}px;
    width: 100%;
    border-left: none;
    ...
    (list) => {
      return list.map((item, index) => (
        <SlideItem key={item.title} index={index} curTheme={curTheme} className="slide-item">
          <Text
            style={isMobile ? textStyle.R_12R : textStyle.R_14R}
